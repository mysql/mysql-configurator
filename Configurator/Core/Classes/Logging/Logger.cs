...
namespace MySql.Configurator.Core.Classes.Logging
{
  /// <summary>
  ...
  public sealed class Logger
  {
    ...
    private Logger()
    {
      ...
      if (ConsoleMode)
      {
        const string LISTENER_NAME = "mysql-console";
        if (LogToConsole)
        {
          listener = new ConsoleTraceListener { Name = LISTENER_NAME };
        }
        else
        {
          listener = new LoggerListener { Name = LISTENER_NAME };
        }
      }
      else
      {
        listener = new LoggerListener { Name = "mysql-gui" };
      }

      Source.Listeners.Add(listener);
      SetTraceListenerOption(_writeTimeStamp, TraceOptions.DateTime);
    }
    ...
    /// <param name="writeTimeStamp">Flag indicating whether a datetime is written along with error messages output to the log.</param>
    public static void Initialize(string homeDirectory, string logFileBaseName, bool consoleMode, bool logToConsole, string traceSourceName, bool writeTimeStamp)
    {
      if (Initialized)
      {
        return;
    ...
    /// <summary>
    /// Sets or un-sets the given <see cref="TraceOptions"/> to the log <see cref="TraceListener"/>.
    /// </summary>
    /// <param name="option">A <see cref="TraceOptions"/> flag.</param>
    /// <param name="set">A flag indicating if the flag is set or unset.</param>
    public static void SetTraceListenerOption(TraceOptions option, bool set)
    {
      if (!_lazyLogger.IsValueCreated)
      {
        throw new LoggerPropertySetException();
      ...
      _lazyLogger.Value.SetTraceListenerOption(set, option);
    }
    ...
    /// <param name="set">A flag indicating if the flag is set or unset.</param>
    /// <param name="option">A <see cref="TraceOptions"/> flag.</param>
    private void SetTraceListenerOption(bool set, TraceOptions option)
    {
      foreach (var listener in Source.Listeners.Cast<TraceListener>().Where(listener => listener.Name.IndexOf("mysql", StringComparison.OrdinalIgnoreCase) >= 0))
      {
        if (set)
        {
