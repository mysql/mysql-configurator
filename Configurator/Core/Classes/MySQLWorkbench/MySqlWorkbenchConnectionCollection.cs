...
namespace MySql.Configurator.Core.Classes.MySqlWorkbench
{
  ...
  public class MySqlWorkbenchConnectionCollection : XmlRepository, IList<MySqlWorkbenchConnection>
  {
    ...
    /// </summary>
    private readonly List<MySqlWorkbenchConnection> _connections;

    /// <summary>
    /// Internal collection of <see cref="MySqlWorkbenchConnectionExtraParameters"/> objects.
    /// </summary>
    private readonly MySqlWorkbenchConnectionExtraParametersCollection _extraParameters;
    ...
    public MySqlWorkbenchConnectionCollection(bool useMySqlWorkbenchConnectionsFile, bool createDefaultConnections)
    {
      ...
      CreateFileUponLoadIfNotFound = !useMySqlWorkbenchConnectionsFile
                                     && !MySqlWorkbench.ConnectionsFileExists;
      CreateFileUponSaveIfNotFound = true;
      RepositoryFilePath = useMySqlWorkbenchConnectionsFile
        ? MySqlWorkbench.ConnectionsFilePath
        : string.Empty;
      IsWorkbenchFile = useMySqlWorkbenchConnectionsFile;
    ...
    {
      get => _createDefaultConnections;
      set => _createDefaultConnections = !UseMySqlWorkbenchConnectionsFile && value;
    }
    ...
    {
      get => _extraParameters.RepositoryFilePath;
      set => _extraParameters.RepositoryFilePath = value;
    }
    ...
    /// <summary>
    /// Removes from the list the first occurrence of a specific <see cref="MySqlWorkbenchConnection"/> object with the given id.
    /// </summary>
    ...
    /// <summary>
    /// Gets a list of <see cref="MySqlWorkbenchConnection"/> that have similar connection attributes to the ones in the given connection.
    /// </summary>
    public List<MySqlWorkbenchConnection> GetSimilarConnections(MySqlWorkbenchConnection sourceConnection)
    {
      return sourceConnection == null
        ? null
        : MySqlWorkbench.Connections.Where(sourceConnection.IsSimilarTo).ToList();
    }
    ...
    public override bool Save(bool throwException)
    {
      ...
      // Reset the connections to a saved status if the save is successful.
      foreach (var conn in _connections.Where(conn => conn.SavedStatus != MySqlWorkbenchConnection.SavedStatusType.InDisk))
      {
    ...
    public bool SaveConnection(MySqlWorkbenchConnection connection)
    {
      ...
      {
        // Add connection to the collection since it's not already there.
        Add(connection);
        connection.SavedStatus = MySqlWorkbenchConnection.SavedStatusType.New;
      }
      else if (connection.SavedStatus == MySqlWorkbenchConnection.SavedStatusType.Updated
               || !updatedConnection.Equals(connection))
      {
        ...
        connection.SavedStatus = MySqlWorkbenchConnection.SavedStatusType.Updated;
      }
    ...
    protected override void CreateCustomXmlElements(ref XmlDocument doc)
    {
      ...
      MySqlWorkbench.CreateRootXmlElements(ref doc, this.RoughSizeOf(), "db.mgmt.Connection");
    }
    ...
    /// <param name="standalone">The value must be either <c>"yes"</c> or <c>"no"</c>. If this is <c>null</c> or <c>string.Empty</c>, the XML declaration will not have a standalone attribute.</param>
    ...
    protected override bool CreateXmlFile(string encoding = null, string standalone = null)
    {
      ...
      foreach (var service in services)
      {
        ...
        var parameters = MySqlServerStartupParameters.GetStartupParameters(winService);
