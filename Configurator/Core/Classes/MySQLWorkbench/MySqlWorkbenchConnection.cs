...
namespace MySql.Configurator.Core.Classes.MySqlWorkbench
{
  ...
  public class MySqlWorkbenchConnection : MySqlWorkbenchConnectionExtraParameters
  {
    ...
    /// </summary>
    public bool IsLocalConnection => IsNamedPipesConnection
                                     || MySqlServerInstance.IsHostLocal(Host);
    ...
    public MySqlWorkbenchConnection Clone(bool cloneId)
    {
    ...
    public MySqlConnectionStringBuilder GetConnectionStringBuilder()
    {
      ...
      {
        case UseSslType.No:
          cs.SslMode = MySqlSslMode.Disabled;
    ...
    public override int GetHashCode()
    {
      unchecked
      {
        ...
        hashCode = (hashCode * Utilities.HASHING_MULTIPLIER) ^ (Id == null ? 0 : Id.GetHashCode());
        return hashCode;
      }
    }
    ...
    /// <summary>
    /// Verifies if the given connection has a similar connection attributes.
    /// </summary>
    /// <param name="otherConnection">A <see cref="MySqlWorkbenchConnection"/> instance.</param>
    /// <returns><c>true</c> if the given connection has a similar connection attributes, <c>false</c> otherwise.</returns>
    public bool IsSimilarTo(MySqlWorkbenchConnection otherConnection)
    {
      ...
      if (otherConnection.ConnectionMethod != ConnectionMethod)
      {
        return false;
      }
      ...
      {
        case ConnectionMethodType.LocalUnixSocketOrWindowsPipe:
          return string.Equals(UnixSocketOrWindowsPipe, otherConnection.UnixSocketOrWindowsPipe, StringComparison.OrdinalIgnoreCase);

        case ConnectionMethodType.Tcp:
        case ConnectionMethodType.XProtocol:
          host = Host;
          otherHost = otherConnection.Host;
          port = Port;
          otherPort = otherConnection.Port;
          break;
      ...
      if (IsLocalConnection
          && otherConnection.IsLocalConnection
          && port == otherPort)
      {
        return true;
      }

      return string.Equals(host, otherHost, StringComparison.OrdinalIgnoreCase)
             && port == otherPort;
    }
    ...
    internal new XmlElement ToElement(XmlDocument doc)
    {
      ...
      XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "hostName", "string", IsNamedPipesConnection ? LOCAL_IP : Host);
      if (!IsNamedPipesConnection)
      {
        XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "port", "int", Port);
      }
      else
      {
        XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "socket", "string", UnixSocketOrWindowsPipe);
      }

      XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "userName", "string", UserName);
      XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "password", "string", string.Empty);

      if (!string.IsNullOrEmpty(DefaultSchema))
      {
        XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "schema", "string", DefaultSchema);
      }

      if (!string.IsNullOrEmpty(SslCertificationAuthorityFile))
      {
        XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "sslCA", "string", SslCertificationAuthorityFile);
      }

      if (!string.IsNullOrEmpty(SslClientCertificateFile))
      {
        XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "sslCert", "string", SslClientCertificateFile);
      }

      if (!string.IsNullOrEmpty(SslKeyFile))
      {
        XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "sslKey", "string", SslKeyFile);
      }

      if (UseCompression)
      {
        XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "CLIENT_COMPRESS", "int", 1);
      }

      if (UseAnsiQuotes)
      {
        XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "useAnsiQuotes", "int", 1);
      }

      if (ConnectionTimeout != DEFAULT_CONNECTION_TIMEOUT)
      {
        XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "timeout", "int", ConnectionTimeout);
      }

      switch (UseSsl)
      {
        case UseSslType.No:
          XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "useSSL", "int", 0);
          break;

        case UseSslType.IfAvailable:
          XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "useSSL", "int", 1);
          break;

        case UseSslType.Require:
          XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "useSSL", "int", 2);
          break;

        case UseSslType.RequireAndVerifyCertificationAuthority:
          XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "useSSL", "int", 3);
          break;

        case UseSslType.RequireAndVerifyIdentity:
          XmlRepository.AppendPropertyValueToElement(doc, parameterValues, "useSSL", "int", 4);
          break;
      }
    ...
    protected new XmlElement GetParameterValuesElementWithBaseConnectionParent(XmlDocument doc)
    {
      ...
      XmlRepository.InsertPropertyValueToElementAfter(doc, connectionElement, link, "isDefault", "int", 0);
      XmlRepository.InsertPropertyValueToElementAfter(doc, connectionElement, link, "hostIdentifier", "string", HostIdentifier);
