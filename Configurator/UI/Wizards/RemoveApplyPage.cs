...
namespace MySql.Configurator.UI.Wizards
{
  public partial class RemoveApplyPage : BaseConfigureRemoveApplyPage
  {
    ...
    /// </summary>
    protected ServerInstallationManager ServerInstallationManager { get; private set; }
    ...
    public override void Activate()
    {
      ...
      foreach (var serverInstallation in removeWizard.ProductsToRemove)
      {
        serverInstallation.Controller.UpdateRemoveSteps();
        var executingSteps = serverInstallation.Controller.RemoveSteps.Where(step => step.Execute).ToList();
        if (!showExpandCollapseButtons && executingSteps.Count > 1)
        {
          showExpandCollapseButtons = true;
        }

        var removeStepControl = new RemoveStepControl(executingSteps, serverInstallation.Controller)
        {
          Label = $"{serverInstallation.NameWithVersion}({ServerInstallation.ARCHITECTURE.ToString().ToLower()})",
    ...
    {
      if (ServerInstallationManager != null
          && !OperationExecuting)
      {
        ServerInstallationManager.Cancel();
      }
    ...
    /// </summary>
    /// <param name="args">The <see cref="ServerInstallationStatusEventArgs"/> instance containing the event data.</param>
    public virtual void ServeInstallationFinished(ServerInstallationStatusEventArgs args)
    {
      switch (args.Status)
      {
      ...
      AddToLogTab(GetCurrentStepControl(args.ServerInstallation).Item1, args.ToString());
    }
    ...
    public virtual void ServeInstallationStarted(ServerInstallationStatusEventArgs args)
    {
    ...
    /// </summary>
    /// <param name="args">The <see cref="ServerInstallationStatusEventArgs"/> instance containing the event data.</param>
    public virtual void ServeInstallationStatusChanged(ServerInstallationStatusEventArgs args)
    {
      var tuple = GetCurrentStepControl(args.ServerInstallation);
      if (tuple == null)
      {
        return;
      }
      ...
      if (args.Status == ServerInstallationStatus.Progress)
      {
    ...
    {
      Wizard.Log = ServerInstallationManager.GetLog();
    ...
    public override void UnsubscribeEvents()
    {
      ...
      _parentRemoveStepControl.Controller.ConfigurationStatusChanged -= controller_ConfigurationStatusChanged;
        ...
        /// <summary>
        /// Gets the <see cref="RemoveStepControl"/> matching the specified <see cref="ServerInstallation"/>.
        /// </summary>
        /// <param name="serverInstallation">The server installation to look for in the remove step control list.</param>
        /// <returns>A tuple containing the index of the remove step control along with the remove step control instance.</returns>
        protected Tuple<int, RemoveStepControl> GetCurrentStepControl(ServerInstallation serverInstallation)
    {
      ...
      for (int index = 0; index < RemoveStepsFlowLayoutPanel.Controls.Count; index++)
      {
        ...
        if (removeStepControl.Controller.ServerInstallation == serverInstallation)
        {
    ...
    /// <summary>
    /// Appends the specified text to the log tab. 
    /// </summary>
    ...
    private void AddToLogTab(int index, string message)
    {
      ...
      ServerInstallationManager.AppendLog($"{index + 1}: {message}");
    }
    ...
    private void Initialize()
    {
      ...
      ServerInstallationManager = new ServerInstallationManager(true);
      ServerInstallationManager.QueueStarted += QueueStarted;
      ServerInstallationManager.QueueFinished += QueueFinished;
      ServerInstallationManager.ServerInstallationStarted += ServeInstallationStarted;
      ServerInstallationManager.ServerInstallationFinished += ServeInstallationFinished;
      ServerInstallationManager.StatusChanged += ServeInstallationStatusChanged;
    }
    ...
    /// <param name="sender">The source of the event.</param>
    /// <param name="e">The <see cref="EventArgs"/> instance containing the event data.</param>
    private void ServerInstallationUninstall(object sender, EventArgs e)
    {
      UninstallProduct();
    }
    ...
    /// <summary>
    /// Adds the specified package to the the queue of server installations marked for uninstall.
    /// </summary>
    /// <param name="serverInstallation">The server installation to add.</param>
    private void SetServerInstallation(ServerInstallation serverInstallation)
    {
      ServerInstallationManager.ResetServerInstallationList();
      ServerInstallationManager.QueueServerInstallation(serverInstallation, ServerInstallationAction.Remove);
      UpdateStatus(GetCurrentStepControl(serverInstallation).Item2);
    }
    ...
    /// <summary>
    /// Updates the UI once all server installations have been uninstalled.
    /// </summary>
    ...
    private void UninstallProduct()
    {
      ...
      SetServerInstallation(CurrentController.ServerInstallation);
      ServerInstallationManager.EnableServerInstallation(CurrentController.ServerInstallation, true);
      ServerInstallationManager.Start();
    ...
    private void UpdateStatus(RemoveStepControl removeStepControl)
    {
      ...
      var serverInstallation = removeStepControl.Controller.ServerInstallation;
      string status = string.Empty;
      if (ServerInstallationManager.Contains(serverInstallation))
      {
        status = ServerInstallationManager.GetStatus(serverInstallation);
      }
